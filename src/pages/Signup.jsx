...
const SignUp = ({type}) => {
  ...
  const validateField = (field, value) => {
    ...
    switch (field) {
      case "name": newErrors.name = value.trim() ? "" : "Name is required"; break;
      case "email": newErrors.email = /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(value) ? "" : "Invalid email"; break;
      case "phone": newErrors.phone = /^\d{7,14}$/.test(value) ? "" : "Enter a valid phone number"; break;
      case "password": newErrors.password = value ? "" : "Password is required"; break;
      case "confirmPassword": newErrors.confirmPassword = value === formData.password ? "" : "Passwords do not match"; break;
      case "role": newErrors.role = value ? "" : "Please select a role"; break;
      default: break;
    }
    setErrors(newErrors);
  };
 ...
 const notifyOTP = (msg = "Verification OTP sent to your registered email!") => {
     toast.success(msg, {
       position: "top-right",
       autoClose: 3000,
       hideProgressBar: false,
...
// }, [status]);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (Object.values(errors).every(err => !err) && Object.values(formData).every(field => field)) {
      try {
      
        const response = await fetch("https://bizplorers-backend.onrender.com/api/auth/register", {
          method: "POST",
...
// };
  ...
  return (
    ...
    <div className="flex justify-center">
        <form onSubmit={handleSubmit} className="bg-slate-100 p-6 rounded-lg shadow-md w-full max-w-md">
          <h2 className="text-2xl font-bold mb-6 text-center">Register</h2>

          <input type="text" name="name" placeholder="Name" value={formData.name} onChange={handleChange} className="w-full mb-2 p-2 border rounded" />
          {errors.name && <p className="text-red-500 text-sm">{errors.name}</p>}

          <input type="email" name="email" placeholder="Email Address" value={formData.email} onChange={handleChange} className="w-full mb-2 p-2 border rounded" />
...
/>
          ...
          <input type="password" name="password" placeholder="Enter Password" value={formData.password} onChange={handleChange} className="w-full mb-2 p-2 border rounded" />
    ...
    >
      ...
      <header className="fixed top-0 left-0 right-0 flex justify-between items-center px-4 py-3 bg-white shadow-md z-10">
        ...
        <nav className="hidden md:flex gap-8">
          <Link to="/homepage" className="text-xl hover:text-blue-600">About Us</Link>
          <Link to="/dashboard" className="text-xl hover:text-blue-600">Services</Link>
          <Link to="/ask-ai" className="text-xl hover:text-blue-600">Seller</Link>
          <Link to="/homepage" className="text-xl hover:text-blue-600">Buyer</Link>
