...
if(type==='modal'){
    if (step === 3) {
      if (!formData.typeOfBuyer.trim()) newErrors.typeOfBuyer = 'Type Of Buyer is required';
    ...
    if (step === 4) {
      if (!formData.businessCategories.length) newErrors.businessCategories = 'Business Category is required';
      if (!formData.ticketSizeMin.trim()) newErrors.ticketSizeMin = 'Minimum Ticket Size is required';
      if (!formData.ticketSizeMax.trim()) newErrors.ticketSizeMax = 'Maximum Ticket Size is required';
      if (!formData.businesslocationCountry.trim()) newErrors.businesslocationCountry = 'Business Location is required';
      if (!formData.businesslocationCities.length) newErrors.businesslocationCities = 'At least one City is required';
      if (!formData.openToPreRevenue.trim()) newErrors.openToPreRevenue = 'Please select an option';
      if (!formData.openToPreBreakeven.trim()) newErrors.openToPreBreakeven = 'Please select an option';
        if (!formData.revenueSizeMin.trim()) newErrors.revenueSizeMin = 'Minimum Revenue is required';
      if (!formData.revenueSizeMax.trim()) newErrors.revenueSizeMax = 'Maximum Revenue is required';
    }

    if (step === 5) {
    
      if (!formData.metric.trim()) newErrors.metric = 'Metric is required';
      if (!formData.maxMultiple.trim()) newErrors.maxMultiple = 'Max Multiple is required';
      if (!formData.preferredArrangement.length) newErrors.preferredArrangement = 'Preferred Arrangement is required';
    }
  }
  else{
     if (step === 1) {
...
console.log("id----",id);
   const dataToSend = type === 'modal'
  ? { ...formData, brokerId: id }
  : { ...formData }


    try {
      console.log("data------formdata ",dataToSend);
      const response = await fetch('https://bizplorers-backend.onrender.com/api/buyer/add_detail', {
        method: 'POST',
...
{type==='modal'? alert('Buyer Created Successfully!'):
  ...
  useEffect(() => {
    if (type === 'modal') {
      setStepsList(['PERSONAL DETAILS', 'OTP VERIFICATION','BASIC DETAILS', 'PREFERENCE DETAILS', 'TRANSACTION DETAILS']);
    } else {
      setStepsList(['BASIC DETAILS', 'PREFERENCE DETAILS', 'TRANSACTION DETAILS']);
    }
  }, [type]); 
  ...
  return (
    <div className='bg-gray-100 min-h-screen'>
      ...
      <div className={`flex justify-center max-h-screen ${type === 'modal' ? 'pt-0' : 'pt-[7%]'}`}>
        <div className="bg-white px-2 py-2 rounded shadow-md w-full  h-full">
       <Stepper step={step} steps={stepsList} />
         {type === 'modal' ? (
  <>
    {step === 1 && <SignUp type={'modal'}/>}
     {step === 2 && <OTPVerification type={'modal'}/>}
    {step === 3 && <StepOne formData={formData} handleChange={handleChange} errors={errors} />}
    {step === 4 && <StepTwo formData={formData} handleChange={handleChange} errors={errors} />}
    {step === 5 && <StepThree formData={formData} handleChange={handleChange} errors={errors} />}
  </>
) : (
  <>
