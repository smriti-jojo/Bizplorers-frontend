...
// };
  ...
  if (step === 3) {
    ...
    if (!formData.cofounder_linkedin.trim()) {
      ...
      newErrors.cofounder_linkedin = 'Cofounder LinkedIn profile is required';
    }
  ...
  if (step === 5) {
    ...
    // }
    if (!formData.preferredArrangement || formData.preferredArrangement.length === 0) {
...
console.log("id----",id);
  ...
  : { ...formData }
    ...
    try {
      ...
      const response = await fetch('https://bizplorers-backend.onrender.com/api/seller/add_detail', {
        method: 'POST',
...
{type==='modal'? alert('Seller Created Successfully!'):
    ...
    useEffect(() => {
      if (type === 'modal') {
        setStepsList(['PERSONAL DETAILS', 'OTP VERIFICATION','COMPANY DETAILS', 'FINANCIAL DETAILS', 'TRANSACTION DETAILS']);
      } else {
        setStepsList(['COMPANY DETAILS', 'FINANCIAL DETAILS', 'TRANSACTION DETAILS']);
      }
    }, [type]); 
